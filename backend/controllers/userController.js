...
// Search and list all software products in the marketplace
...
// Complete a checkout checkout session on cart items
export const handleCheckout = async (req, res, next) => {
  try {
    const { cartItems } = req.body; // Expects array: [{ softwareId, planType }]
    const userId = req.user.id;
    ...
    // Loop through each item in the cart to verify and record subscriptions
    for (const item of cartItems) {
      ...
      // Fetch latest price details from software table directly using model method
      ...
      // Compute plan length expiration date
      ...
      // Record subscription in the database
...
// Retrieve active software subscription licenses for the logged-in client
